...
export default async function ApplicationsPage({ params }: { params: { id: string } }) {
    /* fetch top 3 applicants from DB */
    const topThreeApplicants: ApplicationList = [];

    const supabase = createClient();
    const { data, error } = await supabase
        .from("applications")
        .select("id,name,score,rationale")
        .order("score", { ascending: false })
        .range(0, 2);

    if (data !== null) {
        topThreeApplicants.push(...data);
    } else {
        console.error({ error });
    }
                ...
                {topThreeApplicants.length === 0 ? (
                    ...
                    <div className="grid gap-6 md:gap-8 lg:gap-10">
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 lg:gap-10">
                            {topThreeApplicants.map((applicant) => (
                                <>
                                    <div className="bg-gray-100 rounded-lg p-4 md:p-6 flex flex-col items-start gap-4">
                                        ...
                                        <p className="text-sm text-gray-500 line-clamp-3">
                                            John is an experienced software engineer with a strong background in
                                            full-stack web development. He has a proven track record of delivering
                                            high-quality, scalable applications and has expertise in React, Node.js, and
                                        ...
                                        <div className="mt-auto w-full">
                                            <Link
                                                href={`/dashboard/jobs/${params.id}/applications/${applicant.id}`}
                        ...
                        <div className="flex justify-center">
                            <Link
                                href={`/dashboard/jobs/${params.id}/applications/all`}
